package org.launchcode.sprinklespre.controllers;

public class CourseCountroller {
    //TODO: Set up CourseRepository
    //TODO: Add get mapping - to display all courses at index by ID
    //TODO: Add post mapping - after a user enrolls in a course using a form
    //TODO: Add get mapping - set up a method to display one course by ID at the path course/course_id
    //TODO: Figure out how to format post data as JSON - how do we pull from repo to serve to front end
}
